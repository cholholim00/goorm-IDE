...
```bash
팀미션: 코로나19 대시보드/
├── data/  # 전처리가 완료된 통합 데이터
│   └── daily_covid_data.csv    
│   └── regional_covid_data.csv
│ 
├── 시군구별_월별_확진자_사망_발생현황.csv
├── 연령별_년도별_통합.csv            
├── 예방접종_통계_현황_통합.csv      
├── 일별_발생_성별_통합.csv   
├── project_reset.py # 데이터 전처리 자동화 파이프라인
│ 
├── A_team.py # 국내 확진자/사망자 추이 (라인 차트)
├── B_team.py # 지역별 확진 현황 (지도 그래프 또는 막대 그래프)
├── C_team.py # 백신 접종률 vs 중증화율 상관관계 (산점도)
├── main.py   # 대시보드 실행 메인 파일 (Streamlit)
└── README.md # 프로젝트 설명서
